...
abstract class AbstractTextInput(
    ...
    val placeholderColor = BasicState(InstallerPalette.TEXT_DARK)
    ...
    var linePadding = 0f
    val lineHeightWithPadding: Float
        get() = lineHeight + linePadding
    ...
    protected var targetVerticalScrollingOffset = 0f
        set(value) {
            // Since we added padding, disallow the target scroll offset to be bigger than the difference in height,
            // This prevents us from scrolling higher and revealing the padding below the lowest line, which should be ignored
            val heightDifference = getHeight() - visualLines.size * lineHeightWithPadding + linePadding
            field = value.coerceAtLeast(-heightDifference)
        }
    ...
    init {
        ...
        setColor(InstallerPalette.TEXT)
        ...
        onKeyType { typedChar, keyCode ->
            ...
            if (keyCode == UKeyboard.KEY_ESCAPE) {
                ...
                val newVisualPos = if (cursor.line == 0) {
                    ...
                    screenPosToVisualPos(currX, currY - lineHeightWithPadding)
                }
                ...
                showOptionMenu(getLeft() + posX, getTop() + posY)
            }
        }
        ...
        onMouseScroll {
            ...
            targetVerticalScrollingOffset =
                (targetVerticalScrollingOffset + it.delta.toFloat() * lineHeightWithPadding).coerceIn(heightDifference, 0f)
            it.stopPropagation()
        }
        ...
        onMouseClick { event ->
            ...
            showOptionMenu(event.absoluteX, event.absoluteY)
            event.stopImmediatePropagation()
        }

        // Handles selection of text
        onMouseClick { event ->
        ...
        onMouseDrag { mouseX, mouseY, mouseButton ->
            ...
            if (currentTime - lastSelectionMoveTimestamp > 50) {
                if (mouseY <= 0) {
                    targetVerticalScrollingOffset =
                        (targetVerticalScrollingOffset + lineHeightWithPadding * getTextScale()).coerceAtMost(0f)
                    lastSelectionMoveTimestamp = currentTime
                } else if (mouseY >= getHeight()) {
                    val heightDifference =
                        (getHeight() - visualLines.size * lineHeightWithPadding * getTextScale() + linePadding * getTextScale()).coerceAtMost(0f)
                    targetVerticalScrollingOffset =
                        (targetVerticalScrollingOffset - lineHeightWithPadding * getTextScale()).coerceIn(heightDifference, 0f)
                    lastSelectionMoveTimestamp = currentTime
                } else if (mouseX <= 0) {
    ...
    fun setText(text: String) = apply {
        val newText = filterOutDisallowedCharacters(text)
    ...
    protected open fun commitTextOperation(operation: TextOperation) {
        // Apply the operation, get the updated text
        operation.redo()
        val updatedText = getText()

        // If the text is within the length allowed by [maxLength], save the operation, otherwise undo
        if (maxLength < 0 || updatedText.length <= maxLength) {
    ...
    private fun filterOutDisallowedCharacters(text: String): String {
    ...
    protected fun commitTextAddition(newText: String) {
        val text = filterOutDisallowedCharacters(newText)
    ...
    protected open fun drawSelectedText(matrixStack: UMatrixStack, text: String, left: Float, right: Float, row: Int) {
        UIBlock.drawBlock(
            ...
            getTop().toDouble() + (lineHeightWithPadding * row * getTextScale()) + verticalScrollingOffset,
            right.toDouble() - horizontalScrollingOffset,
            getTop().toDouble() + (lineHeightWithPadding * ((row + 1) * getTextScale())) + verticalScrollingOffset
        )
        if (text.isNotEmpty()) {
            getFontProvider().drawString(
                matrixStack,
    ...
    protected inner class AddTextOperation(val newText: String, private val startPos: LinePosition) :
        TextOperation() {
        override fun redo() {
    ...
    companion object {
        ...
        fun isAllowedCharacter(chr: Char): Boolean {

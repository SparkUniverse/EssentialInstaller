...
// Heavily adapted from AlphaEffect
class LeftAndRightGradientFadeEffect(private val gradientPadding: Double) : Effect() {
    ...
    override fun beforeDraw(matrixStack: UMatrixStack) {
        if (resources.textureId == -1) error("LeftAndRightTextFadeEffect has not yet been setup or has already been cleaned up! ElementaVersion.V4 or newer is required for proper operation!")
    ...
    override fun afterDraw(matrixStack: UMatrixStack) {
        ...
        val height = bottom - top
        val widthLeftGradient = gradientPadding
        val xLeftGradient = left
        val widthRightGradient = gradientPadding
        val xRightGradient = right - gradientPadding

        if (height == 0.0 || !shader.usable) {
            return
        }
        ...
        fun renderGradient(x: Double, width: Double, isLeftGradient: Boolean) {
            worldRenderer.beginWithActiveShader(UGraphics.DrawMode.QUADS, UGraphics.CommonVertexFormats.POSITION_TEXTURE_COLOR)
            worldRenderer.pos(matrixStack, x, top + height, 0.0).tex(0.0, 0.0).color(red, green, blue, if (isLeftGradient) 1f else 0f).endVertex()
            worldRenderer.pos(matrixStack, x + width, top + height, 0.0).tex(1.0, 0.0).color(red, green, blue, if (isLeftGradient) 0f else 1f).endVertex()
            worldRenderer.pos(matrixStack, x + width, top, 0.0).tex(1.0, 1.0).color(red, green, blue, if (isLeftGradient) 0f else 1f).endVertex()
            worldRenderer.pos(matrixStack, x, top, 0.0).tex(0.0, 1.0).color(red, green, blue, if (isLeftGradient) 1f else 0f).endVertex()
            worldRenderer.drawDirect()
        }

        renderGradient(xLeftGradient, widthLeftGradient, true)
        renderGradient(xRightGradient, widthRightGradient, false)

        shader.unbind()
    }

    private class Resources(effect: LeftAndRightGradientFadeEffect) : PhantomReference<LeftAndRightGradientFadeEffect>(effect, referenceQueue), Closeable {
        var textureId = -1
        ...
        companion object {
            val referenceQueue = ReferenceQueue<LeftAndRightGradientFadeEffect>()
    ...
    companion object {
        ...
        private fun initShader() {
            ...
            if (!shader.usable) {
                println("Failed to load LeftAndRightTextFadeEffect shader")
                return
            }
